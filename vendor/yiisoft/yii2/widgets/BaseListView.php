...
abstract class BaseListView extends Widget
{
    ...
    public function init()
    {
        ...
        if ($this->emptyText === null) {
            $this->emptyText = Yii::t('yii', 'No se han encontrado resultados');
        }
    ...
    public function renderSummary()
    {
        ...
        if (($pagination = $this->dataProvider->getPagination()) !== false) {
            ...
            if (($summaryContent = $this->summary) === null) {
                return Html::tag($tag, Yii::t('yii', 'Mostrar <b>{begin, number}-{end, number}</b> de <b>{totalCount, number}</b> {totalCount, plural, one{item} other{items}}.', [
                        'begin' => $begin,
                        'end' => $end,
                        'count' => $count,
