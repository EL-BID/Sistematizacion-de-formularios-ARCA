 ...
 * @since 1
 */
class ExcelARCA extends \yii\base\Widget
{
	/**
	 ...
	 * saving the xls file to download or to path
	 */
	public function writeFileHTML($sheet,$page)
	{
		if (!isset($this->format))
			$this->format = 'Excel2007';
                                    ob_end_clean();
		...
		if ($this->mode == 'export') 
		{
                    ...
                    if ($this->isMultipleSheet) {
                            ...
                            foreach ($this->models as $title => $models) {
                                    if($index>0){$sheet->createSheet($index);}
...
//                            throw new InvalidConfigException('Config models must be set');
                   ...
                   //Se organizan styles para el codigo de excel
                   ...
                   if ($this->isMultipleSheet) {
                        $index = 0;
                        $sheet->removeSheetByIndex();
                        //$worksheet = [];
                        foreach ($this->models as $title => $html) {
                                $objPHPExcel2= new \PHPExcel();
                                $tmpfile = tempnam(sys_get_temp_dir(), 'html');
                                file_put_contents($tmpfile, $html);
                                $excelHTMLReader = \PHPExcel_IOFactory::createReader('HTML');
                                $objPHPExcel2->setActiveSheetIndex(0); 
                                $objPHPExcel2= $excelHTMLReader->load($tmpfile); 
                                $objWorkSheet=$objPHPExcel2->getActiveSheet()->setTitle($title); 
                                $sheet->addExternalSheet($objWorkSheet); 
	    			...
	    			$index++;
                        ...
                        $tmpfile = tempnam(sys_get_temp_dir(), 'html');
                        file_put_contents($tmpfile, $this->models);
                        $excelHTMLReader = \PHPExcel_IOFactory::createReader('HTML');
                        $sheet = $excelHTMLReader->load($tmpfile);
                        $worksheet = $sheet->getActiveSheet();
                        unlink($tmpfile);
