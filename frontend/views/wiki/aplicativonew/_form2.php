<script>
function setTop(){
   var page="<?php echo Yii::$app->getUrlManager()->createUrl('aplicativonew/grilla')  ; ?>";
   $.ajax({
           url: page,
           type: "POST",
           data: {
...
use frontend\models\Provinciasnew;                         //Agregando modelo provincias
use frontend\models\Formato_new;                            //Agregando modelo de formato
        ...
        <tr>
                ...
                echo Html::dropDownList('provincia',[],
                     $list,
                      ['prompt'=>'Seleccione una provincia','id'=>'provincia',
                          'onchange'=>'$.post("index.php?r=aplicativonew/listado&id='.'"+$(this).val(),function(data){
                              $("#cantones").html(data);
                          });']
                      ); 
                ?>                 
            ...
            <td>
                ...
                echo Html::dropDownList('cantones',[],[],['prompt'=>'Seleccione un canton','id' => 'cantones',
                         'onchange'=>'$.post("index.php?r=aplicativonew/listadop&id='.'"+$(this).val(),function(data){
            ...
            <td>
                 <?php
                    ...
                    echo Html::dropDownList('formato',[],
                         ArrayHelper::map(Formato_new::find()->all(),'id_formato','fullFormat'),
                         ['prompt'=>'Seleccione una formato','id'=>'formato','onchange'=>'$.post("index.php?r=aplicativonew/listadoe&id='.'"+$(this).val(),function(data){
                             var res = data.split("::");
                             $("#estados").html(res[0]);
                             $("#periodos").html(res[1]);
                         });']
