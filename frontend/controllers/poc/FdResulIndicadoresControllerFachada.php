...
use common\models\poc\FdResulIndicadoresSearch;
...
/**
 * FdResulIndicadoresControllerFachada implementa la verificaicon de los valores, consulta información para aplicar reglas de negocio, y transacciones conforme las acciones para el modelo FdResulIndicadores.
 */
class FdResulIndicadoresControllerFachada extends FachadaPry
{
    /**
     * @inheritdoc
     */
    ...
    {
                $searchModel = new FdResulIndicadoresSearch();
     ...
     * @return mixed
     */
    public function actionCreate($id,$request,$isAjax,$id_cnj_rpta)
    {
        $model = new FdResulIndicadores();
        if(!empty($id_cnj_rpta)){
            $model->id_conjunto_respuesta =$id_cnj_rpta;
            $model->periodo=date('Y')-1;
        }
        
        $_verifica = FdResulIndicadores::findOne(['id_indicador'=>$id,'id_conjunto_respuesta'=>$id_cnj_rpta]);        
        if(count($_verifica)>0)
        {
            
            $model = $this->findModel($_verifica['id_resul_indicadores']);            
        }
    ...
    /**
     * Encuentra el modelo de FdResulIndicadores basado en el valor de la primary key.
     * Si no encuentra respuesta, arroja una excepcion 404 HTTP.
     * @param integer $id
     * @return FdResulIndicadores el modelo cargado
     * @throws NotFoundHttpException si no puede ser encontrada la respuesta
     */
    protected function findModel($id)
    {
                    if (($model = FdResulIndicadores::findOne($id)) !== null) {
                        return $model;
                    } else {
     ...
     * @return type  FdResulIndicadores     */
    protected function findModelByParams($params)
    {
        if (($model = FdResulIndicadores::findAll($params)) !== null) {
            return $model;
        }
        else{
            return null;
    ...
    /**
     * Funcion visible para los objetos que necesiten una consulta al modelo FdResulIndicadores     
     * @param type $params contiene array con valores de query a columnas de la tabla
     * @return type FdResulIndicadores     */
     public function cargaFdResulIndicadores($params){
