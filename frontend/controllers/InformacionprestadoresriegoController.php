...
use Yii;
use frontend\controllers\InformacionprestadoresriegoControllerFachada;
 ...
 */
class InformacionprestadoresriegoController extends ControllerPry
{
  //private $facade =    InformacionprestadoresriegoControllerFachada;
    /**
     * @inheritdoc
     */
    ...
    public function actionIndex($id_fmt,$id_conj_prta,$id_cnj_rpta,$estado,$provincia,$cantones,$hashbutton)
    {
       ...
       if(!empty($hashbutton))
       {
            ...
            $list_entriego = $this->getEntidadesriego($provincia,$cantones);
       }        
       if(!empty($_post))
         {           
    ...
    public function getEntidadesriego($provincia,$cantones){
        
        if(!empty($provincia) && !empty($cantones) )
        {
             $formatoPOST = Yii::$app->db->createCommand('select ent.nombre_entidad as enti, res.id_conjunto_respuesta  as id_conj_res, CONCAT(dgr.nombres_prestador_servicio,\'/\',ubi.cod_provincia,\'/\',ubi.cod_canton) as presta
                                                    from entidades as ent 
                                                    inner join fd_conjunto_respuesta as res on ent.id_entidad = res.id_entidad
                                                    inner join fd_datos_generales_riego as dgr on dgr.id_conjunto_respuesta = res.id_conjunto_respuesta
