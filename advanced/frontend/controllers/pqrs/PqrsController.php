...
class PqrsController extends ControllerPry
{
  //private $facade =    PqrsControllerFachada;
    ...
    public function actionIndex()
    {
        ...
        if(!in_array(Yii::$app->user->identity->codRols[0]->cod_rol,[28,29,27,21,30,31,32])){
            return $this->redirect(['site/index']);  
        }
    ...
    public function actionCreate($tipo=null)
    {
        ...
        //Validando loguin =================================================================================
        if(!empty(Yii::$app->user->identity->codRols[0]->cod_rol) and !in_array(Yii::$app->user->identity->codRols[0]->cod_rol,[28,29,27,21,30,31,32])){
            return $this->redirect(['site/index']);  
        }
...
//    }
    ...
    public function actionExcel($id_pqr=null,$nombre_hoja=null){
        //Datos del pqr y el proceso
        $facadePrs = new PqrsControllerFachada;
        $pqrs = $facadePrs->getPcproceso($id_pqr);
    ...
    public function actionPdfDenuncia($id_pqr=null){
        ...
        if( count($pqrs)>1){
            foreach ($pqrs as $pqr){
                $_stringhtmlC =$_stringhtmlC. $_callhelper->gen_htmlPqrsDenunciaPdf($pqr);
            }
            $nombre_formato=$nombre_formato."multiple".date("YmdHis");
        }else{
                $_stringhtmlC =$_stringhtmlC. $_callhelper->gen_htmlPqrsDenunciaPdf($pqrs);
                $nombre_formato=$nombre_formato.$pqrs->num_consecutivo.'_'.date("YmdHis");
        }
        

        $_stringhtml = $_callhelper->gen_htmlPqrsDenunciaPdf($pqrs);
    ...
    public function actionPdfQueja($id_pqr=null){
        ...
        $_stringhtml = $_callhelper->gen_htmlPqrsQuejaPdf($pqrs);
        ...
        $_stringhtml = $_callhelper->gen_htmlPqrsPeticionPdf($pqrs);
    ...
    public function actionPdfSugerencia($id_pqr=null){
        ...
        $_stringhtml = $_callhelper->gen_htmlPqrsSugerenciaPdf($pqrs);
