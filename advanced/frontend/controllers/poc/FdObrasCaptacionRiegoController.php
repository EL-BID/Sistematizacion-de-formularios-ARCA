...
use Yii;
use frontend\controllers\poc\FdObrasCaptacionRiegoControllerFachada;
...
/**
 * FdObrasCaptacionRiegoController implementa las acciones a través del sistema CRUD para el modelo FdObrasCaptacionRiego.
 */
class FdObrasCaptacionRiegoController extends ControllerPry
{
  //private $facade =    FdObrasCaptacionRiegoControllerFachada;
    /**
     * @inheritdoc
     */
    public function behaviors()
    {
        $facade =  new  FdObrasCaptacionRiegoControllerFachada;
 ...
 /**
    ...
    public function actionIndexdetcapitulo($id_fmt, $id_version, $id_conj_prta, $id_cnj_rpta, $id_capitulo, $id_prta, $id_rpta, $numerar, $nom_prta, $migadepan, $estado, $capitulo, $provincia, $cantones, $parroquias, $periodos, $antvista, $focus)
    {
        ...
        if (empty($id_rpta)) {
            ...
            $rpta_exists = FdRespuesta::find()
                          ...
                          ->andwhere(['=', 'fd_respuesta.id_tpregunta', '19'])
        ...
        $facade = new FdObrasCaptacionRiegoControllerFachada();
        //FdInvolucradoSearch
        $_busquedap = ['FdObrasCaptacionRiegoSearch' => [
            ...
            if(!empty($_respuesta_anidada) and $_respuesta_anidada->ra_entero>0){
                ...
                if($_respuesta_anidada->ra_entero>0  and $_respuesta_anidada->ra_entero<=$_conteodata){
                    ...
                    Yii::$app->getSession()->setFlash('error', [
                           'type' => 'error',
                           'message' => 'No es posible agregar más obras de captación',
                       ]);                    
                }else{
    ...
    /**
     * Presenta un dato unico en el modelo FdObrasCaptacionRiego.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Crea un nuevo dato sobre el modelo FdObrasCaptacionRiego .
    ...
    public function actionCreate()
    {
       ...
       $model = $modelE['model'];
        if ($modelE['create'] == 'True') {
            ...
            return  $this->redirect(['progress', 'urlroute' => 'view', 'id' => $model->id_obracaptacion]);		
    ...
    $_nombrevalor = FdTipoSelect::find()
            ->where(['nom_tselect' => 'TIPO OBRA CAPTACIÓN'])
            ->one();    
    $valor_tselect= FdOpcionSelect::find()->where(['id_tselect'=>$_nombrevalor->id_tselect])->all();

    $_nombrevalorestado = FdTipoSelect::find()
            ->where(['nom_tselect' => 'Estado'])
            ->one();        
    $valor_tselectestado =FdOpcionSelect::find()->where(['id_tselect'=>$_nombrevalorestado->id_tselect])->all();

    $_nombrevalorubicacion = FdTipoSelect::find()
            ->where(['nom_tselect' => 'UBICACION OBRAS'])
            ->one();        
    $valor_tselectubicacion = FdOpcionSelect::find()->where(['id_tselect'=>$_nombrevalorubicacion->id_tselect])->all();
    ...
    public function actionUpdatedetcapitulo($id, $id_fmt, $id_version, $id_conj_prta, $id_cnj_rpta, $id_capitulo, $id_prta, $id_rpta, $nom_prta, $numerar, $migadepan, $estado, $capitulo, $provincia, $cantones, $parroquias, $periodos, $antvista, $focus)
    {
        ...
        $bande = false;
        if (!empty($id)) {
            $_obracapta = FdObrasCaptacionRiego::find()
                    ->where(['id_obracaptacion' => $id])
                    ->one();
            $valor_capta = $_obracapta->tipo_obra;
            $valor_ubica = $_obracapta->ubicacion_obra;
            ...
            $otrosu = FdOpcionSelect::find()
                    ->where(['id_opcion_select' => $valor_ubica])
                    ->one();
            $nom_otrosu = $otrosu->nom_opcion_select;
            if ($nom_otrosu == "Otro") {
                $bande = true;
            }
