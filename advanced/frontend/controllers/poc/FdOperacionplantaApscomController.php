...
/**
 * FdOperacionplantaApscomController implementa las acciones a través del sistema CRUD para el modelo FdOperacionplantaApscom.
 */
class FdOperacionplantaApscomController extends ControllerPry
{
  //private $facade =    FdOperacionplantaApscomControllerFachada;
    /**
     * @inheritdoc
     */
    public function behaviors()
    {
        $facade =  new  FdOperacionplantaApscomControllerFachada;
    ...
    {                
        ...
        $facade = new FdOperacionplantaApscomControllerFachada();
        //FdInvolucradoSearch
        $_busquedap = ['FdOperacionplantaApscomSearch' => [
...
//        //Enviando dato para habilitar o no boton adicionar
    ...
    public function actionCreate()
    {
       ...
       $model = $modelE['model'];
        if ($modelE['create'] == 'True') {
            ...
            return  $this->redirect(['progress', 'urlroute' => 'view', 'id' => $model->id_operacionplanta]);		
    ...
    public function actionCreatedetcapitulo($id_fmt, $id_version, $id_conj_prta, $id_cnj_rpta, $id_capitulo, $id_prta, $id_rpta, $nom_prta, $numerar, $migadepan, $estado, $capitulo, $provincia, $cantones, $parroquias, $periodos, $antvista, $focus,$idjunta)
    {
        ...
        $_nombrevalor = FdTipoSelect::find()
         ...
         $_nombreprob = FdTipoSelect::find()
                 ->where(['nom_tselect' => 'PROBLEMAS OPERACIÓN PLANTA'])
                 ->one();         
    ...
    public function actionUpdatedetcapitulo($id, $id_fmt, $id_version, $id_conj_prta, $id_cnj_rpta, $id_capitulo, $id_prta, $id_rpta, $nom_prta, $numerar, $migadepan, $estado, $capitulo, $provincia, $cantones, $parroquias, $periodos, $antvista, $focus,$idjunta)
    {
        ...
        $bande=false;
        if (!empty($id)) {
        $_opera = FdOperacionplantaApscom::find()
                ->where(['id_operacionplanta' => $id])
                ->one();
        $valor_opera = $_opera->problemas;

        $otros = FdOpcionSelect::find()
                ->where(['id_opcion_select' => $valor_opera])
                ->one();
        $nom_otros ="";
        if(!empty($otros))
          $nom_otros = $otros->nom_opcion_select;
        if ($nom_otros == "Otro (indicar)") {
            $bande = true;
        }
    }
