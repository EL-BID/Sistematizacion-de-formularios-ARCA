<?php

namespace backend\controllers\poc;

use Yii;
use backend\controllers\poc\FdPeriodoFormatoControllerFachada;
use common\controllers\controllerspry\ControllerPry;
use yii\web\NotFoundHttpException;
use yii\filters\VerbFilter;
use yii\helpers\Html;	//Para presentar la barra de espera
use yii\helpers\Url;	//Para presentar la barra de espera
use yii\jui\ProgressBar;

/**
 * FdPeriodoFormatoController implementa las acciones a través del sistema CRUD para el modelo FdPeriodoFormato.
 */
class FdPeriodoFormatoController extends ControllerPry
{
  //private $facade =    FdPeriodoFormatoControllerFachada;
    /**
     * @inheritdoc
     */
    public function behaviors()
    {
        $facade =  new  FdPeriodoFormatoControllerFachada;
        return $facade->behaviors();
    }
	
    
	
    /**Accion para la barra de progreso y render de nuevo a la vista
    Ubicación: @web = frontend\web....*/

    public function actionProgress($urlroute=null,$id=null){
            $facade =  new  FdPeriodoFormatoControllerFachada;
            echo $facade->actionProgress($urlroute,$id);
    }

	
	
    /**
     * Listado todos los datos del modelo FdPeriodoFormato que se encuentran en el tablename.
     * @return mixed
     */
    public function actionIndex()
    {
         $facade =  new  FdPeriodoFormatoControllerFachada;
        $dataAndModel= $facade->actionIndex(Yii::$app->request->queryParams);
        
        return $this->render('index', [
            'searchModel' => $dataAndModel['searchModel'],
            'dataProvider' => $dataAndModel['dataProvider'],
        ]);
    }

    /**
     * Presenta un dato unico en el modelo FdPeriodoFormato.
     * @param integer $id_formato
     * @param integer $id_periodo
     * @return mixed
     */
    public function actionView($id_formato, $id_periodo)
    {
        $facade =  new  FdPeriodoFormatoControllerFachada;
        return $this->render('view', [
            'model' => $facade->actionView($id_formato, $id_periodo),
        ]);
    }

    /**
     * Crea un nuevo dato sobre el modelo FdPeriodoFormato .
     * Si se crea correctamente guarda setFlash, presenta la barra de progreso y envia a view con la confirmación de guardado.
     * @return mixed
     */
    public function actionCreate()
    {
       $facade =  new  FdPeriodoFormatoControllerFachada;
       $modelE= $facade->actionCreate(Yii::$app->request->post(),Yii::$app->request->isAjax);
       $model = $modelE['model'];
        if ($modelE['create'] == 'True') {
			
            Yii::$app->session->setFlash('FormSubmitted','2');
            return  $this->redirect(['progress', 'urlroute' => 'view', 'id_formato' => $model->id_formato, 'id_periodo' => $model->id_periodo]);		
			
        }elseif($modelE['create']=='Ajax'){
             return $this->renderAjax('create', [
                        'model' => $model
            ]);
        } 
        else {
            return $this->render('create', [
                'model' => $model,
            ]);
        }
    }

    /**
     * Modifica un dato existente en el modelo FdPeriodoFormato.
     * Si se modifica correctamente guarda setFlash, presenta la barra de progreso y envia a view con la confirmación de guardado..
     * @param integer $id_formato
     * @param integer $id_periodo
     * @return mixed
     */
    public function actionUpdate($id_formato, $id_periodo)
    {
        $facade =  new  FdPeriodoFormatoControllerFachada;
        $modelE= $facade->actionUpdate($id_formato, $id_periodo,Yii::$app->request->post(),Yii::$app->request->isAjax);
        $model = $modelE['model'];

        if ($modelE['update'] == 'True') {
            
            Yii::$app->session->setFlash('FormSubmitted','1');		
            return $this->redirect(['progress', 'urlroute' => 'view', 'id_formato' => $model->id_formato, 'id_periodo' => $model->id_periodo]);
            
        }elseif($modelE['update']=='Ajax'){
            return $this->renderAjax('update', [
                        'model' => $model
            ]);
        } 
        else {
            return $this->render('update', [
                'model' => $model,
            ]);
        }
    }

    /**
     * Deletes an existing FdPeriodoFormato model.
     * If deletion is successful, the browser will be redirected to the 'index' page.
     * @param integer $id_formato
     * @param integer $id_periodo
     * @return mixed
     */
    public function actionDeletep($id_formato, $id_periodo)
    {
        $facade =  new  FdPeriodoFormatoControllerFachada;
        $facade->actionDeletep($id_formato, $id_periodo);

        return $this->redirect(['index']);
    }

    
}
